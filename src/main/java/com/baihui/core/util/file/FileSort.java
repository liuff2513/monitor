...
/**
 * 读取目录文件并且按照时间进行排序
 * Created by ziyu.zhang on 2016/4/29 13:01
 */
public class FileSort {
    ...
    public static List getFiles(String dirname)throws Exception{
        ...
        if(dir.exists()){
            ...
            for(int i=0;i<files.length;i++){
                //获取当文件最后修改时间
                String creatime=FileSort.format("yyyy-MM-dd hh:mm:ss",new Date(files[i].lastModified()));
    ...
    /**
     * 进行文件排序时间
     * Created by ziyu.zhang on 2016/4/29 13:02
     * @Param 
     * @return 
     */
    ...
    public static void main(String[] args){
        try {
            //读取文件
            List filenames=FileSort.getFiles("D:/");
            if(filenames!=null)
